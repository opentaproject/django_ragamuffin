...
class OpenAI(TestCase):
    ...
    def test_upload_processed_path_uses_chunks(self):
    ...
    def test_upload_processed_path_normalizes_jsonl(self):
        file_path = os.path.join(settings.OPENAI_UPLOAD_STORAGE, "nn", "abc123", "records.jsonl")
        self.assertEqual(
            upload_processed_relative_path(file_path),
            os.path.join("nn", "abc123", "chunks", "records.json"),
        )
    ...
    def test_add_file_by_name(self):
        ...
        # Create Assistant and add the file by absolute path
        aname = randstring('A')
        assistant = Assistant(name=aname)
        assistant.save()
        assistant.add_file_by_name(tmp_path)

        # Verify the file name appears in assistant.files()
        names = [ i[1] for i in assistant.files()   ] # list of (pk, name)
        print(f"NAMES = {names}")
        print(f"filename = {filename}")
        assert any(filename == n for n in names), f"Expected {filename} in {names}"

        # Cleanup temp file; stored copy remains in OPENAI_UPLOAD_STORAGE
        try:
            os.remove(tmp_path)
